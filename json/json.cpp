...
using namespace yazi::json;
    ...
    Json::Json(Type type) : m_type(type)//根据类型的构造函数
    {
        switch(m_type)
        {
            case json_null:
    ...
    Json::Json(const Json& other)//根据值构造
    {
       copy(other);
    }
...
//运算符重载的实现
    ...
    void Json::append (const Json & other)
    {
        ...
        (m_value.m_array)->push_back(other);//如果已经是数据类型，调用push-back
    }
 ...
 void Json::copy(const Json & other)
 {
         ...
         switch(m_type)
        {
            ...
            case json_double:
                ...
                break;
            case json_string:
                m_value.m_string=other.m_value.m_string;//对内存浅拷贝
                break;
            case json_array:
...
bool Json::asBool() const
{
    if(m_type != json_bool)
    {   
        throw new std::logic_error("type error,not boll value");
    }
    return m_value.m_bool;
}
